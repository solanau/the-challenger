...
exports.fetchChallengesForEvent = async (req, res) => {
    try {
        const challengeQuerySnapshot = await db.collection(challengeCollection).get();
        const challenges: ChallengePayload[] = [];
        challengeQuerySnapshot.forEach(
            (doc)=>{
                const data: any = doc.data();
                if (data.eventPubkey === req.params.eventPubkey) challenges.push(data);
            }
        );
...
exports.fetchChallengeById = async (req, res) => {
    try {
        const challengeQuerySnapshot = await db.collection(challengeCollection).where('id', '==', req.params.id).get();
        const challenges: ChallengePayload[] = [];
        challengeQuerySnapshot.forEach(
            (doc)=>{
                const data: any = doc.data();
...
exports.fetchChallengeByKey = async (req, res) => {
    try {
        const challengeQuerySnapshot = await db.collection(challengeCollection).where('key', '==', req.params.key.toString()).get();
        const challenges: ChallengePayload[] = [];
        challengeQuerySnapshot.forEach(
            (doc)=>{
                const data: any = doc.data();
...
exports.createNewChallenge = async function(req, res) {
    ...
    if (req.params.masterApiKey != MASTER_API_KEY) {
        ...
        const challengePubkey = Keypair.generate().publicKey;
        try {
            rawChallenge = req.body
            if (!rawChallenge['authorName']) rawChallenge['authorName'] = "None";
        } catch (error) {
            console.log(error);
            res.status(500).send(PayloadError());
        }
        // try {
        //     challengePubkey = (await createChallenge(
        //         connection, 
        //         WALLET, 
        //         PRESTIGE_PROGRAM_ID, 
        ...
        // }
        try {
            const challenge: ChallengePayload = { pubkey: challengePubkey.toBase58(), ...rawChallenge };
            const newDoc = await db.collection(challengeCollection).add(challenge);
            res.status(201).send({
...
exports.updateChallenge = async function(req, res) {
    ...
    if (req.params.masterApiKey != MASTER_API_KEY) {
        ...
        try {
            ...
            const newDoc = await db.collection(challengeCollection).doc(req.params.id).set(challenge);
